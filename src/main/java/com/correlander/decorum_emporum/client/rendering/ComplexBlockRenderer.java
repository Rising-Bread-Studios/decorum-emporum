package com.correlander.decorum_emporum.client.rendering;
...
public class ComplexBlockRenderer implements BlockEntityRenderer<ComplexBlockEntity> {
    public static final ResourceLocation LIGHT = new ResourceLocation(DecorumEmporum.MODID, "block/light");

    public ComplexBlockRenderer(BlockEntityRendererProvider.Context context) {
    }
    @Override
    public void render(ComplexBlockEntity be, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSrc, int combinedLight, int combinedOverlay) {
        ...
        if (h != null) {
            ...
            if (!stack.isEmpty()) {
                ...
                itemRenderer.renderStatic(stack, ItemDisplayContext.FIXED, LightTexture.FULL_BRIGHT, combinedOverlay, poseStack, bufferSrc, Minecraft.getInstance().level, 0);
                poseStack.popPose();
                poseStack.translate(0, .5f, 0);
                renderBillboardQuadBright(poseStack, bufferSrc.getBuffer(RenderType.translucent()), 0.5f, LIGHT);
                poseStack.popPose();
            }
        }
    }
